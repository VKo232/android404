...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String gLogin = getIntent().getStringExtra("gLogin");
        String gPass = getIntent().getStringExtra("gPass");
        if (gLogin != null) {
            if (gLogin.length() > 0) {
                showProgress(true);
                mAuthTask = new UserLoginTask(gLogin, gPass);
        ...
        protected void onPostExecute(final Boolean success) {
            ...
            if (success) {
                try {
                    ...
                    if (customerObj != null) {
                        String customerObjSt = new ObjectMapper().writeValueAsString(customerObj);
                        myIntent.putExtra("customerObjSt", customerObjSt); //Optional parameters
                        String accountObjListSt = new ObjectMapper().writeValueAsString(accountObjList);
                        myIntent.putExtra("accountObjListSt", accountObjListSt); //Optional parameters
                        myIntent.putExtra("failMsg", "");
                        startActivity(myIntent);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
